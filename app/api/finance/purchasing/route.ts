...
type InvoiceRow = { id: string; order_id: string; order_number?: string | null; vendor_name?: string | null; invoice_number: string; invoice_date: string; due_date: string; supply_amount: number; tax_amount: number; total_amount: number; matched_receipt_amount: number; status: string; exception_reason: string; payment_request_id: string; created_by: string };
...
async function ensureSchema() {
  await db.batch([
    ...
    db.prepare(`CREATE TABLE IF NOT EXISTS finance_purchase_invoices (
      id TEXT PRIMARY KEY NOT NULL, order_id TEXT NOT NULL, invoice_number TEXT NOT NULL, invoice_date TEXT NOT NULL,
    ...
    db.prepare("CREATE UNIQUE INDEX IF NOT EXISTS idx_finance_purchase_invoice_number ON finance_purchase_invoices(invoice_number)"),
...
const toInvoice = (row: InvoiceRow) => ({ id: row.id, orderId: row.order_id, orderNumber: row.order_number ?? "", vendorName: row.vendor_name ?? "", invoiceNumber: row.invoice_number, invoiceDate: row.invoice_date, dueDate: row.due_date, supplyAmount: row.supply_amount, taxAmount: row.tax_amount, totalAmount: row.total_amount, matchedReceiptAmount: row.matched_receipt_amount, status: row.status, exceptionReason: row.exception_reason, paymentRequestId: row.payment_request_id, createdBy: row.created_by });
...
export async function POST(request: Request) {
  ...
  if (resource === "invoice") {
    ...
    if (await db.prepare("SELECT id FROM finance_purchase_invoices WHERE invoice_number = ?").bind(invoiceNumber).first()) return Response.json({ error: "같은 매입 인보이스번호가 이미 있습니다." }, { status: 409 });
    ...
    await db.prepare(`INSERT INTO finance_purchase_invoices
      (id, order_id, invoice_number, invoice_date, due_date, supply_amount, tax_amount, total_amount,
        matched_receipt_amount, status, exception_reason, payment_request_id, created_by, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '', ?, ?, ?)`)
      .bind(id, orderId, invoiceNumber, invoiceDate, dueDate, supplyAmount, taxAmount, supplyAmount + taxAmount,
