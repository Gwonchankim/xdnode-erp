...
type ApprovalStep = { id: string; requestId: string; stepOrder: number; stepName: string; approverRole: string; approverEmployeeId: string; status: string; comment: string; actedBy: string; actedAt: number | null };
...
type ApprovalData = {
  ...
  summary: { pendingMine: number; requestedByMe: number; active: number };
  ...
  return <>
    ...
    {open && <>
      ...
      <section className="approval-center" role="dialog" aria-modal="true" aria-label="전자결재 센터">
        ...
        <div className="approval-summary"><article><span>내 결재 대기</span><strong>{data?.summary.pendingMine ?? 0}</strong></article><article><span>내가 기안한 진행 건</span><strong>{data?.summary.requestedByMe ?? 0}</strong></article><article><span>전체 진행 건</span><strong>{data?.summary.active ?? 0}</strong></article></div>
        ...
        <div className="approval-layout">
          <aside className="approval-request-list">{loading && <div className="approval-empty">불러오는 중입니다.</div>}{!loading && requests.map((item) => <button type="button" key={item.id} className={selected?.id === item.id ? "active" : ""} onClick={() => setSelectedId(item.id)}><span><em>{moduleLabels[item.module]} · {item.typeLabel}</em><time>{item.dueDate || "기한 없음"}</time></span><strong>{item.title}</strong><small>{employeeLabel(item.requesterEmployeeId)} · {statusLabels[item.status] ?? item.status}</small></button>)}{!loading && !requests.length && <div className="approval-empty">이 조건의 결재 문서가 없습니다.</div>}</aside>
