...
const STORAGE_KEY = "xdnode-compensation-roster-v1";
...
function blankEmployee(year: number, month: number): Employee {
  return { id: crypto.randomUUID(), name: "", department: "", title: "", birthDate: "", joinDate: `${year}-${String(month).padStart(2, "0")}-01`, leaveDate: "", probationMonths: 0, annualSalary: 0, manualBasic: false, meal: 200_000, car: 0, child: 0, monthly: {} };
}
...
function parseRoster(rows: ImportedCell[][], year: number, month: number) {
  ...
  for (const row of rows.slice(headerIndex + 1)) {
    ...
    result.push({ id: crypto.randomUUID(), name, department: String(get(row, "department") ?? "").trim(), title: String(get(row, "title") ?? "").trim(), birthDate: dateString(get(row, "birthDate")), joinDate: dateString(get(row, "joinDate")), leaveDate: dateString(get(row, "leaveDate")), probationMonths: probationValue === true || /90|true/i.test(String(probationValue ?? "")) ? 3 : 0, annualSalary, manualBasic: annualSalary <= 0 && basic > 0, meal: numberValue(get(row, "meal")) || (annualSalary ? 200_000 : 0), car: numberValue(get(row, "car")), child: numberValue(get(row, "child")), monthly: annualSalary <= 0 && basic > 0 ? { ...monthly, [monthKey(year, month)]: { ...monthly[monthKey(year, month)], basic } } : monthly });
  }
  return result;
}
...
function WageCalculator() {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        const data = JSON.parse(saved) as { employees?: Employee[]; columns?: typeof columns; standards?: typeof standards };
  ...
  useEffect(() => { if (hydrated) localStorage.setItem(STORAGE_KEY, JSON.stringify({ employees, columns, standards })); }, [employees, columns, standards, hydrated]);
  ...
  return <div className="wage-calculator">
    <section className="compensation-toolbar panel">
      ...
      <div className="wage-period-controls"><label>연도<input type="number" value={year} onChange={(event) => setYear(Number(event.target.value) || now.getFullYear())} /></label><label>월<select value={month} onChange={(event) => setMonth(Number(event.target.value))}>{Array.from({ length: 12 }, (_, index) => <option key={index + 1} value={index + 1}>{index + 1}월</option>)}</select></label><label>단수 처리<select value={rounding} onChange={(event) => setRounding(event.target.value as Rounding)}><option value="round">반올림</option><option value="up">올림</option><option value="down">내림</option></select></label></div>
    </section>
