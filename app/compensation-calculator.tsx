...
function WageCalculator() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/hr/compensation?period=${key}`).then(async (response) => {
      ...
      setEmployees(payload.run?.employees ?? []);
  ...
  return <div className={`wage-calculator${locked ? " locked" : ""}`}>
    ...
    {run && <section className={`compensation-run-status ${locked ? "confirmed" : "draft"}`}><strong>{locked ? "확정된 임금안" : "작성 중인 임금안"}</strong><span>{locked ? "HR 급여관리에 반영됨 · 수정하기 후 재확정하면 기존 월 기록을 덮어씁니다." : "변경 내용을 저장한 뒤 확정하면 HR 급여관리로 전송됩니다."}</span></section>}
