...
function WageCalculator() {
  ...
  return <div className={`wage-calculator${locked ? " locked" : ""}`}>
    ...
    <section className="panel wage-table-panel"><header><div><p>PAYROLL CALCULATION</p><h2>{year}년 {month}월 임금 계산 결과</h2></div><span>열 제목 오른쪽 경계를 드래그해 너비를 조정할 수 있습니다.</span></header><div className="wage-table-scroll"><table style={{ width: tableWidth }}><colgroup>{tableColumns.map((column) => <col key={column.key} style={{ width: columnWidths[column.key] ?? DEFAULT_COLUMN_WIDTHS[column.key] }} />)}</colgroup><thead><tr>{tableColumns.map((column) => <th key={column.key}>{column.label}<button type="button" className="column-resizer" aria-label={`${column.label || "작업"} 열 너비 조절`} onPointerDown={(event) => beginColumnResize(event, column.key)} /></th>)}</tr></thead><tbody>{rows.map((row) => { const employee = row.employee; const monthly = employee.monthly[key] ?? {}; return <tr key={employee.id} className={row.days ? "" : "inactive"}><td><input aria-label="성명" value={employee.name} onChange={(event) => updateEmployee(employee.id, { name: event.target.value })} placeholder="성명" /></td>{isColumnVisible("department") && <td><input list="wage-departments" aria-label="부서" value={employee.department} onChange={(event) => updateEmployee(employee.id, { department: event.target.value })} placeholder="부서" /></td>}{isColumnVisible("title") && <td><input list="wage-titles" aria-label="직책" value={employee.title} onChange={(event) => updateEmployee(employee.id, { title: event.target.value })} placeholder="직책" /></td>}{isColumnVisible("joinDate") && <td><input type="date" aria-label="입사일" value={employee.joinDate} onChange={(event) => updateEmployee(employee.id, { joinDate: event.target.value })} /></td>}{isColumnVisible("leaveDate") && <td><input type="date" aria-label="퇴사일" value={employee.leaveDate} onChange={(event) => updateEmployee(employee.id, { leaveDate: event.target.value })} /></td>}{isColumnVisible("days") && <td className="calculated">{row.days}일</td>}{isColumnVisible("probation") && <td><select aria-label="수습 기간" value={employee.probationMonths} onChange={(event) => updateEmployee(employee.id, { probationMonths: Number(event.target.value) })}>{[0, 1, 2, 3, 6, 12].map((value) => <option key={value} value={value}>{value ? `${value}개월` : "—"}</option>)}</select></td>}{isColumnVisible("annualSalary") && <td><input className="money-input" type="number" aria-label="연봉" value={employee.annualSalary || ""} onChange={(event) => updateEmployee(employee.id, { annualSalary: Number(event.target.value) || 0 })} /></td>}{isColumnVisible("basic") && <td><div className="basic-cell">{employee.manualBasic ? <input className="money-input" type="number" aria-label="기본급 직접 입력" value={monthly.basic ?? employee.basePay ?? ""} onChange={(event) => updateMonthly(employee.id, "basic", Number(event.target.value) || 0)} /> : <b>{money(row.basic)}</b>}<label title="기본급 직접 입력"><input type="checkbox" checked={employee.manualBasic} onChange={(event) => { updateEmployee(employee.id, { manualBasic: event.target.checked }); if (event.target.checked && monthly.basic === undefined) updateMonthly(employee.id, "basic", employee.basePay || row.basic); }} />직접</label></div></td>}{(["meal", "car", "child"] as const).map((field) => isColumnVisible(field) && <td key={field}><div className="allowance-cell"><b>{money(row[field])}</b><input type="checkbox" aria-label={`${field} 지급`} checked={employee[field] > 0} onChange={(event) => setAllowance(employee.id, field, event.target.checked)} /></div></td>)}{(["incentive", "bonus"] as const).map((field) => isColumnVisible(field) && <td key={field}><input className="money-input" type="number" aria-label={field} value={monthly[field] ?? ""} onChange={(event) => updateMonthly(employee.id, field, Number(event.target.value) || 0)} /></td>)}{columns.extra && isColumnVisible("extra") && <td><input className="money-input" type="number" aria-label="추가수당" value={monthly.extra ?? ""} onChange={(event) => updateMonthly(employee.id, "extra", Number(event.target.value) || 0)} /></td>}{columns.research && isColumnVisible("research") && <td><input className="money-input" type="number" aria-label="연구수당" value={monthly.research ?? ""} onChange={(event) => updateMonthly(employee.id, "research", Number(event.target.value) || 0)} /></td>}{columns.severance && isColumnVisible("severance") && <td><input className="money-input" type="number" aria-label="퇴직금" value={monthly.severance ?? ""} onChange={(event) => updateMonthly(employee.id, "severance", Number(event.target.value) || 0)} /></td>}<td className="total-cell">{money(row.total)}</td>{columns.welfare && isColumnVisible("welfare") && <td><input className="money-input" type="number" aria-label="복지기금" value={monthly.welfare ?? ""} onChange={(event) => updateMonthly(employee.id, "welfare", Number(event.target.value) || 0)} /></td>}{isColumnVisible("note") && <td><input aria-label="비고" value={monthly.note ?? ""} onChange={(event) => updateMonthly(employee.id, "note", event.target.value)} placeholder="비고" /></td>}<td><button type="button" className="remove-row" aria-label={`${employee.name || "직원"} 삭제`} onClick={() => { if (confirm(`${employee.name || "이 직원"}을 명부에서 삭제할까요?`)) setEmployees((current) => current.filter((item) => item.id !== employee.id)); }}>×</button></td></tr>; })}{!rows.length && <tr><td colSpan={tableColumns.length} className="wage-empty">{run ? "해당 월 임금 대상자가 없습니다." : `${month}월 임금 작성하기를 눌러 HR 인사기록을 불러오세요.`}</td></tr>}</tbody></table><datalist id="wage-departments">{departments.map((value) => <option key={value} value={value} />)}</datalist><datalist id="wage-titles">{titles.map((value) => <option key={value} value={value} />)}</datalist></div><footer><button disabled={Boolean(exporting)} type="button" onClick={() => void exportCurrent()}>{exporting === "month" ? "파일 만드는 중…" : "현재 월 엑셀 내려받기"}</button><button disabled={Boolean(exporting)} type="button" onClick={() => void exportYear()}>{exporting === "year" ? "파일 만드는 중…" : "1~12월 일괄 내려받기"}</button>{locked ? <button disabled={saving} className="payroll-footer-action edit" type="button" onClick={() => void compensationAction("REOPEN")}>수정하기</button> : <button disabled={saving || autoSaving || !employees.length} className="payroll-footer-action confirm" type="button" onClick={() => void compensationAction("CONFIRM")}>{saving ? "처리 중…" : "급여 확정"}</button>}<span>초안은 서버에 저장되고 확정 시 HR 급여관리로 반영됩니다.</span></footer></section>
    </fieldset>
  </div>;
}
