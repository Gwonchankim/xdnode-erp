...
export const financePostingBatches = sqliteTable("finance_posting_batches", {
  id: text("id").primaryKey(), validationId: text("validation_id").notNull().default(""), sourceBatchId: text("source_batch_id").notNull().default(""), batchNumber: text("batch_number").notNull(), sourceType: text("source_type").notNull().default("IMPORT"), reversalOfBatchId: text("reversal_of_batch_id").notNull().default(""),
  status: text("status").notNull().default("DRAFT"), periodFrom: text("period_from").notNull(), periodTo: text("period_to").notNull(), voucherCount: integer("voucher_count").notNull().default(0), lineCount: integer("line_count").notNull().default(0), totalDebit: integer("total_debit").notNull().default(0), totalCredit: integer("total_credit").notNull().default(0), differenceAmount: integer("difference_amount").notNull().default(0),
  approvalRequestId: text("approval_request_id").notNull().default(""), reason: text("reason").notNull().default(""), preparedBy: text("prepared_by").notNull(), approvedBy: text("approved_by").notNull().default(""), postedBy: text("posted_by").notNull().default(""), submittedAt: integer("submitted_at"), approvedAt: integer("approved_at"), postedAt: integer("posted_at"), version: integer("version").notNull().default(1), createdAt: integer("created_at").notNull(), updatedAt: integer("updated_at").notNull(),
}, (table) => [uniqueIndex("idx_finance_posting_validation").on(table.validationId).where(sql`${table.validationId} <> ''`), uniqueIndex("idx_finance_posting_reversal").on(table.reversalOfBatchId).where(sql`${table.reversalOfBatchId} <> ''`), uniqueIndex("idx_finance_posting_batch_number").on(table.batchNumber), index("idx_finance_posting_status_period").on(table.status, table.periodFrom, table.periodTo)]);

export const financePostingVouchers = sqliteTable("finance_posting_vouchers", {
  id: text("id").primaryKey(), batchId: text("batch_id").notNull(), sourceVoucherKey: text("source_voucher_key").notNull(), voucherDate: text("voucher_date").notNull(), period: text("period").notNull(), voucherNumber: text("voucher_number").notNull().default(""), description: text("description").notNull().default(""), sourceReference: text("source_reference").notNull().default(""), status: text("status").notNull().default("DRAFT"), lineCount: integer("line_count").notNull().default(0), totalDebit: integer("total_debit").notNull().default(0), totalCredit: integer("total_credit").notNull().default(0), createdAt: integer("created_at").notNull(), updatedAt: integer("updated_at").notNull(),
}, (table) => [uniqueIndex("idx_finance_posting_source_voucher").on(table.batchId, table.sourceVoucherKey), uniqueIndex("idx_finance_posting_voucher_number").on(table.voucherNumber).where(sql`${table.voucherNumber} <> ''`), index("idx_finance_posting_voucher_period").on(table.period, table.status)]);

export const financePostingLines = sqliteTable("finance_posting_lines", {
  id: text("id").primaryKey(), voucherId: text("voucher_id").notNull(), lineNumber: integer("line_number").notNull(), accountId: text("account_id").notNull(), accountCode: text("account_code").notNull(), accountName: text("account_name").notNull(), partnerId: text("partner_id").notNull().default(""), partnerName: text("partner_name").notNull().default(""), departmentId: text("department_id").notNull().default(""), departmentName: text("department_name").notNull().default(""),
  taxCodeId: text("tax_code_id").notNull().default(""), taxCode: text("tax_code").notNull().default(""), taxCodeName: text("tax_code_name").notNull().default(""), taxReviewStatus: text("tax_review_status").notNull().default("PENDING"), taxReviewNote: text("tax_review_note").notNull().default(""), taxReviewedBy: text("tax_reviewed_by").notNull().default(""), taxReviewedAt: integer("tax_reviewed_at"), description: text("description").notNull().default(""), debitAmount: integer("debit_amount").notNull().default(0), creditAmount: integer("credit_amount").notNull().default(0),
  sourceCanonicalRowId: text("source_canonical_row_id").notNull().default(""), sourceChecksum: text("source_checksum").notNull().default(""), reversalOfLineId: text("reversal_of_line_id").notNull().default(""), createdAt: integer("created_at").notNull(), updatedAt: integer("updated_at").notNull(),
}, (table) => [uniqueIndex("idx_finance_posting_line_number").on(table.voucherId, table.lineNumber), uniqueIndex("idx_finance_posting_source_row").on(table.sourceCanonicalRowId).where(sql`${table.sourceCanonicalRowId} <> ''`), uniqueIndex("idx_finance_posting_reversal_line").on(table.reversalOfLineId).where(sql`${table.reversalOfLineId} <> ''`), index("idx_finance_posting_tax_review").on(table.taxReviewStatus, table.taxCodeId)]);

export const financePostingEvents = sqliteTable("finance_posting_events", {
...
export const hrOfferRequests = sqliteTable("hr_offer_requests", {
  ...
  onboardedBy: text("onboarded_by").notNull().default(""),
