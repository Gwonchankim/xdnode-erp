...
test("cash series use observed closing balances without inventing unsupported months", () => {
  ...
  assert.equal(daily.points[0].startDate, financeCurrentData.balanceTrend[daily.points.length - 1]?.date);
  ...
  assert.equal(monthly.points.at(-1)?.value, financeCurrentData.balanceTrend[0].balance);
